...
export function Navbar() {
  ...
  const links = [[labels[0], "#solution"], [labels[1], "#features"], [labels[2], "#integrations"], [labels[3], "#pricing"]];
  ...
  return <header className="site-header"><div className="container nav-inner"><BrandMark/><nav className="desktop-nav" aria-label="Main navigation">{links.map(([label, href]) => <a href={href} key={href}>{label}</a>)}</nav><div className="desktop-language">{switcher}</div><a href="#contact" className="button button-sm desktop-cta">{labels[4]}</a><button className="menu-button" type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-menu" aria-label={labels[5]}><span/><span/><span/></button></div><nav id="mobile-menu" className={`mobile-nav ${open ? "is-open" : ""}`} aria-label="Mobile navigation">{links.map(([label, href]) => <a href={href} key={href} onClick={() => setOpen(false)}>{label}</a>)}<div className="mobile-language">{switcher}</div><a href="#contact" className="button" onClick={() => setOpen(false)}>{labels[4]}</a></nav></header>;
}
